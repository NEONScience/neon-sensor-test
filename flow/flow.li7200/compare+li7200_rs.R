...
compare.stream = function(stream, interval = "01", test.week = "1", Percentages = TRUE){
  ...
  day_range_dt = data.table::data.table(
    ...
    dplyr::select(date) # %>% 
    # dplyr::filter(date != "2021-04-22") %>%  # 28 QFs 
    # dplyr::filter(date != "2021-05-07") %>%  # 40 QFs
    # dplyr::filter(date != "2021-05-11") # 41 QFs
  ...
  if(stream %in% c("pres")){
  ...
  } else if(stream %in% c("frt00Samp", "rtioMoleDryCo2Raw", "rtioMoleDryH2oRaw", "veloZaxsErth")){
    
    #############################################  frt00Samp Comparison         ######################################
    #############################################  rtioMoleDryCo2Raw Comparison ######################################
    
    message(paste0("Grabbing ", stream ," Comparison"))
    ecte_soni_join = data.table::data.table()
    for(day in day_range){
      day = as.Date(day, origin = "1970-01-01")
      # Set Dyanmic File Names for A and B sensors... 
      Li7200A.file = paste0(base.dir, "Li7200/NEON.D10.HQTW.IP4.00200.001.ecte.", day,".expanded.h5")
      ...
      if(stream != "veloZaxsErth") {
        ...
        # For veloZaxsErth only
        file_of_interest = ls_of_file %>% 
          dplyr::filter(name == stream) %>% 
          dplyr::filter(stringr::str_detect(string = group, pattern = "qfqm")) %>% 
          dplyr::filter(stringr::str_detect(string = group, pattern = paste0(interval,"m"))) %>% 
          tidyr::unite(col = file_path, c("group", "name"), sep = "/") %>% dplyr::select(file_path)
        
        # Read in A and B sensors data
        ecte_soni = rhdf5::h5read(file = Li7200A.file, name = file_of_interest$file_path[1]) %>%
          dplyr::mutate(phase = phase_insert)
        
        ecte_soni_join = data.table::rbindlist(l = list(ecte_soni_join, ecte_soni))
    ...
    if(stream != "veloZaxsErth"){
      ...
      if(stream %in% c("rtioMoleDryCo2Raw", "rtioMoleDryH2oRaw")){
        
        library(broom)
        ecte_wide = Li7200.dcast %>%
          dplyr::mutate(day = as.Date(timeBgn, origin = "1970-01-01"))
        
        regr_intercept_slope = ecte_wide %>% 
          dplyr::group_by(day) %>% do(regr = lm(`Li7200 (regular)` ~  `Li7200 (RS)`, data = .)) %>% 
          broom::tidy(regr) %>% 
          dplyr::select(day, term, estimate) %>% 
          reshape2::dcast(day ~ term, value.var = "estimate") 
        names(regr_intercept_slope) = c("day", "intercept", "slope")

        regr_rsq = ecte_wide %>% 
          dplyr::group_by(day) %>% do(regr = lm(`Li7200 (regular)` ~  `Li7200 (RS)`, data = .)) %>% 
          broom::glance(regr) %>% 
          dplyr:: select(day, r.squared)
        
        regr_stats = dplyr::full_join(regr_intercept_slope, regr_rsq, by = "day")
        
        ecte_wide_r2 = ecte_wide %>% dplyr::left_join(regr_stats, by = "day") %>% 
          dplyr::mutate(stats = paste0("R^2: ", round(r.squared,4),"\nSlope: ", round(slope,4), "\nInt: ", round(intercept,4)))
  
        plot3 = ggplot(ecte_wide_r2, aes(x = `Li7200 (regular)`, y = `Li7200 (RS)`)) +
          geom_point() + 
          geom_smooth(method = "lm", formula = "y ~ x") +
          scale_y_continuous() +
          geom_text(x=-Inf,y=+Inf, hjust=0, vjust=1.1, aes(label = stats), size = 4.5, family = 'courier', fontface = "plain") +
          ...
          labs(title = "Comparing Li7200 and Li7200 RS Directly During both phases", 
               ...
               y = paste0("Li7200 (RS)      ", ecte_wide_r2$units[1]), 
               x = paste0("Li7200 (Regular)      ", ecte_wide_r2$units[1]) 
          ) +
        ...
        plot3 = ggplot(Li7200.dcast, aes(x = `Li7200 (regular)`, y = `Li7200 (RS)`)) +
          geom_point() + 
    ...
    } else if(stream == "veloZaxsErth"){
      
      return(ecte_soni_join)
    ...
    if(stream == "fluxCo2/turb"){
      ...
      ecte_plot_4 = ggplot(ecte_boxplot_data, aes(x = day, y = difference)) +
        ...
        annotate("rect", xmin = "2021-04-21", xmax = "2021-05-02", ymin = plot_max, ymax = plot_min, alpha = 0.1, fill = "#00cc00") +
        annotate("rect", xmin = "2021-05-02", xmax = "2021-05-17", ymin = plot_max, ymax = plot_min, alpha = 0.1, fill = "blue") +  
        labs(x = "", y = paste0(ecte_boxplot_data$units[1]), fill = "Sensor",
             title =  paste0("Time-series boxplots of raw ", stream, " differences between the Li7200 and Li7200 RS")) +
...
test = compare.stream(stream = "rtioMoleDryH2oRaw")

test[[6]]

soni_qf = compare.stream(stream = "veloZaxsErth")


qf_rain = soni_qf %>% dplyr::mutate(day = as.Date(timeBgn, origin = "1970-01-01")) %>% 
    dplyr::group_by(day) %>% 
    dplyr::summarise(
      percent_qfFinl = round(100*sum(qfFinl, na.rm = TRUE) / 1440,0)
    )
knitr::kable(
  qf_rain
)
...
grab.stats = function(week = "1"){
  ...
  data.6.false = compare.stream(stream = "rtioMoleDryH2oRaw", Percentages = FALSE, test.week = week)
  ...
  data.6.true  = compare.stream(stream = "rtioMoleDryH2oRaw", Percentages =  TRUE, test.week = week)
  

  data.mad.stats.true =   data.table::rbindlist(l = list(data.1.true[[2]],  data.2.true[[2]],  data.3.true[[4]],  data.4.true[[4]],  data.5.true[[4]], data.6.true[[4]]))
  data.mad.stats.false =  data.table::rbindlist(l = list(data.1.false[[2]], data.2.false[[2]], data.3.false[[4]], data.4.false[[4]], data.5.false[[4]], data.6.false[[4]]))
  data.rsmd.stats.true =  data.table::rbindlist(l = list(data.1.true[[3]],  data.2.true[[3]],  data.3.true[[5]],  data.4.true[[5]],  data.5.true[[5]], data.6.true[[5]]))
  data.rsmd.stats.false = data.table::rbindlist(l = list(data.1.false[[3]], data.2.false[[3]], data.3.false[[5]], data.4.false[[5]], data.5.false[[5]], data.6.false[[5]]))
  ...
  data.stats = dplyr::left_join(data.stats.med.mad, data.stats.rmsd.rsq, by = c("streamID", "units")) %>%
    ...
    dplyr::filter(streamID %in% c("fluxCo2/turb", "fluxH2o/turb","rtioMoleDryCo2Raw", "rtioMoleDryH2oRaw"))

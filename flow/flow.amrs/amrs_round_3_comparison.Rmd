...
run_1_duration_dt = soniAmrs01_data %>% 
  dplyr::filter(run == 1) 
run_1_duration = c(lubridate::ymd_hms(min(run_1_duration_dt$timeBgn)), lubridate::ymd_hms(max(run_1_duration_dt$timeBgn)))
run_2_duration_dt = soniAmrs01_data %>% 
  dplyr::filter(run == 2) 
run_2_duration = c(lubridate::ymd_hms(min(run_2_duration_dt$timeBgn)) , lubridate::ymd_hms(max(run_2_duration_dt$timeBgn)))
run_3_duration_dt = soniAmrs01_data %>% 
  dplyr::filter(run == 3) 
run_3_duration = c(lubridate::ymd_hms(min(run_3_duration_dt$timeBgn)) , lubridate::ymd_hms(max(run_3_duration_dt$timeBgn)-1))

# run_3_duration_dt %>% 
#   dplyr::mutate(date = as.Date(timeBgn, origin = "1970-01-01")) %>% 
#   dplyr::group_by(date) %>% 
#   dplyr::summarise(
#     is_na_ang = sum(is.na(angXaxsDeg))/length(angXaxsDeg),
#     is_na_acc = sum(is.na(accXaxs))/length(accXaxs)
#   )
...
acc_axsDiffFilt.prcs <- stats.data %>%
  ...
  dplyr::mutate(setting = ifelse(test = sensor == "soniAmrs02" & test_date %in% seq.Date(from = run_2_duration[1], to = run_2_duration[2], by = "1 day"), yes = "01.04.05", no = NA)) %>% 
  dplyr::mutate(setting = ifelse(test = sensor == "soniAmrs02" & test_date %in% seq.Date(from = run_3_duration[1], to = run_3_duration[2], by = "1 day"), yes = "01.05.01", no = setting)) %>% 
  dplyr::mutate(setting = ifelse(test = sensor == "soniAmrs03" & test_date %in% seq.Date(from = run_2_duration[1], to = run_2_duration[2], by = "1 day"), yes = "01.05.01", no = setting)) %>%
  dplyr::mutate(setting = ifelse(test = sensor == "soniAmrs03" & test_date %in% seq.Date(from = run_3_duration[1], to = run_3_duration[2], by = "1 day"), yes = "01.04.05", no = setting)) %>% 
...
acc_axsDiffFilt.diffMean <- stats.data %>%
  ...
  dplyr::filter(is.na(setting) == FALSE) %>%
  dplyr::filter(is.na(value) == FALSE) %>%
  dplyr::mutate(value = kableExtra::cell_spec(value, "html", color = ifelse(abs(value) <= 0.1, "#00cc00","firebrick"), background = ifelse(setting == "01.04.05", "white","black"), bold = TRUE)) %>% 
  dplyr::filter(test_date >= run_3_duration[1]) %>% 
  reshape2::dcast(sensor + measurement + stat ~ test_date)  
...
avel_axsDeg.prcs <- stats.data %>%
  ...
  dplyr::filter(is.na(setting) == FALSE) %>%
  dplyr::filter(is.na(value) == FALSE) %>%
  dplyr::mutate(value = kableExtra::cell_spec(value, "html", color = ifelse(abs(value) <= 2.0, "#00cc00","firebrick"), background = ifelse(setting == "01.04.05", "white","black"), bold = TRUE)) %>% 
  dplyr::filter(test_date >= run_3_duration[1]) %>% 
  reshape2::dcast(sensor + measurement + stat ~ test_date)  
...
avel_axsDeg.reso <- stats.data %>%
  ...
  dplyr::filter(is.na(setting) == FALSE) %>%
  dplyr::filter(is.na(value) == FALSE) %>%
  dplyr::mutate(value = kableExtra::cell_spec(value, "html", color = ifelse(abs(value) <= 0.2, "#00cc00","firebrick"), background = ifelse(setting == "01.04.05", "white","black"), bold = TRUE)) %>% 
  dplyr::filter(test_date >= run_3_duration[1]) %>% 
  reshape2::dcast(sensor + measurement + stat ~ test_date)

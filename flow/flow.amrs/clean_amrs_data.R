compare_amrs_data = function(){
  ...
  test_data_lookup = aws.s3::get_bucket_df(bucket = sensor_test_bucket, prefix = "sensor/amrs/round_3/") %>% 
    ...
    # dplyr::filter(date == "2021-08-23") %>% # & date != "2021-08-21") %>% 
  ...
  for(i in base::seq_along(test_dates)){
    ...
    # Time regularization is not required when using L0p data. So this section has been commented out
    ...
    message(print(knitr::kable(soniAmrs_final_stats %>% dplyr::filter(stat == "prcs"))))
    
    message(paste0(Sys.time(), ": Saving rmsd.diff.prcs.rsq.reso file!. . ."))
    saveRDS(soniAmrs_final_stats, paste0("~/eddy/neon-sensor-test/data/2021_AMRS_Round_3/stats/", date_i,".RDS"))
    ...
    saveRDS(object = rpt, file = paste0("~/eddy/neon-sensor-test/data/2021_AMRS_Round_3/final_outputs/", date_i, ".RDS"))
  }
}
